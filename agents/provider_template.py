"""
Provider Agent 模板 — 独立技能 + AgentFleet SDK 支付
===================================================
AgentFleet 是纯支付层，不绑定 Agent 的具体技能。
...
# ═══════════════════════════════════════════════════════════════
#  Part 1: 你的独立技能 — 完全不依赖 AgentFleet
# ═══════════════════════════════════════════════════════════════

def my_stock_analysis(symbol: str) -> dict:
    """
    你的核心业务逻辑：股票分析。
    这部分完全独立，与 AgentFleet 无关。
...
# ═══════════════════════════════════════════════════════════════
#  Part 2: AgentFleet SDK — 支付结算层
# ═══════════════════════════════════════════════════════════════

class StockAnalysisAgent:
    """
    一个完整的 Provider Agent。
    组合了自己的技能 (stock analysis) + AgentFleet SDK (支付能力)。
    """

    def __init__(self, api_base: str = "http://127.0.0.1:8765"):
        self.client = AgentFleetClient(api_base=api_base)
    ...
    def onboard_as_api_key(self, name: str, price: float = 5.0) -> str:
        ...
        if api_key:
            ...
            print(f"[AgentFleet] ✅ 注册成功: {r['agent_id']} | API Key: {api_key}")
        return api_key
    ...
    def onboard_as_feishu(self, name: str, app_id: str, app_secret: str, price: float = 5.0) -> str:
        ...
        print(f"[AgentFleet] ✅ 飞书注册成功: {r['agent_id']}")
        return api_key
    ...
    def onboard_as_openclaw(self, name: str, bot_token: str, price: float = 5.0) -> str:
        ...
        print(f"[AgentFleet] ✅ OpenClaw 注册成功: {r['agent_id']}")
        return api_key
    ...
    def serve(self, consumer_id: str, symbol: str, charge: float = None):
        ...
        一次完整的服务流程:
          ...
          2. 通过 AgentFleet SDK 对 Consumer 自动计费
        """
        print(f"\n{'='*50}")
        ...
        try:
            ...
            print(f"[AgentFleet] 💰 计费成功: {charge} credits | TX: {tx.get('transaction_id','')}")
        except Exception as e:
            print(f"[AgentFleet] ❌ 计费失败: {e}")
